...
for m in range(N):
    for n in range(N):
        g_base_pair[m,n] = free_energy_pair(sequence[m],sequence[n],5.69,6.0,4.09) # bp1, bp2, g_AU, g_GU, g_GC

for dg in [5.69, 6.0, 4.09, 1.0, -7.09]:
    if is_circular:
        print str(dg)+' '+str(circular_sequence_derivatives(g_base_pair, 1.0, -7.09, N, dg))
        ...
        print str(dg)+' '+str(linear_sequence_derivatives(g_base_pair, 1.0, -7.09, N, dg)) # g_base_pair, g_loop, g_stack, N, g
