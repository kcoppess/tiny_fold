...
def gradient(param, sequence, index):
    deriv_sum_sq = np.zeros(4)
    M = len(sequence)
    g_base_pair = gm.generator(sequence, param[0], param[1], param[2], M)
    actual_g_base_pair = gm.generator(sequence, p.energies[0], p.energies[1], p.energies[2], M)
    
    bpp_actual = bpp.mccaskill_linear(actual_g_base_pair, g_loop, p.energies[3], M)
    bpp_matrix = bpp.mccaskill_linear(g_base_pair, g_loop, param[3], M)
    residual = bpp_actual - bpp_matrix
    for k in range(4):
        deriv_bpp_matrix = bpp.mccaskill_linear_derivatives(g_base_pair, g_loop, param[3], M, param[k])
        deriv_sum_sq[k] += np.sum(((-2) * residual * deriv_bpp_matrix))
    prior = (p.priors - param)/50.
...
def convergence(param, prev_param): 
    if np.linalg.norm(param - prev_param) < 1e-9:
        return True
    else:
        return False
...
while not convergence(param, prev_param) and k < 20000:
    ...
    for i in index:
        ...
        t += 1
        grad = gradient(param, p.training_sequences[i], i)
